...
public class SetBackgroundImageDialog extends DialogWrapper {
        ...
        UiNotifyConnector.doWhenFirstShown(
                ...
                () ->
                        createTemporaryBackgroundTransform(
                                myPreviewPanel, myPropertyTmp, getDisposable()));
        setupComponents();
        restoreRecentImages();
    ...
    protected Action[] createActions() {
        return ArrayUtil.append(
                super.createActions(),
                new AbstractAction("Clear and Close") {
                    @Override
    ...
    private void setupComponents() {
        ...
        myPathField.addBrowseFolderListener(
                null, null, null, descriptor, TextComponentAccessor.STRING_COMBOBOX_WHOLE_TEXT);
        ...
        for (Enumeration<AbstractButton> e = getFillRbGroup().getElements();
                e.hasMoreElements(); ) {
            AbstractButton button = e.nextElement();
            button.setActionCommand(button.getText());
            button.addItemListener(this::fillOrAnchorChanged);
        }
        for (Enumeration<AbstractButton> e = getAnchorRbGroup().getElements();
                e.hasMoreElements(); ) {
            AbstractButton button = e.nextElement();
            button.setActionCommand(button.getText());
            button.addItemListener(this::fillOrAnchorChanged);
        }
        ChangeListener opacitySync =
                new ChangeListener() {
        ...
        boolean perProject =
                !Comparing.equal(
                        getBackgroundSpec(myProject, getSystemProp(true)),
                        getBackgroundSpec(null, getSystemProp(true)));
    ...
    private AnAction createToggleAction(String target, String text) {
        class A extends IconWithTextAction implements DumbAware, Toggleable {
            ...
            public void update(AnActionEvent e) {
                ...
                e.getPresentation()
                        .putClientProperty(
                                Toggleable.SELECTED_PROPERTY, target.equals(myPreviewTarget));
                super.update(e);
            }
    ...
    private void retrieveExistingValue() {
        ...
        JBIterable<String> possibleValues =
                JBIterable.of(myResults.get(prop))
    ...
    private String calcNewValue() {
        ...
        return path.trim()
                + ","
                + myOpacitySpinner.getValue()
                + ","
                + (type + "," + anchor).toLowerCase(Locale.ENGLISH);
    }
    ...
    private static JPanel createAnchorPanel(@NotNull ButtonGroup buttonGroup) {
        ...
        for (int i = 0; i < names.length; i++) {
            JRadioButton button =
                    new JRadioButton(names[i], values[i] == IdeBackgroundUtil.Anchor.CENTER);
    ...
    @NotNull
    private static JPanel createFillPanel(
            @NotNull ButtonGroup buttonGroup, @NotNull Disposable disposable) {
        ...
        for (int i = 0; i < values.length; i++) {
            ...
            JBPanelWithEmptyText clickablePanel =
                    addClickablePanel(buttonPanel, buttonGroup, radioButton, color);
            createTemporaryBackgroundTransform(
                    clickablePanel,
                    image,
                    values[i],
    ...
    private static JBPanelWithEmptyText addClickablePanel(
